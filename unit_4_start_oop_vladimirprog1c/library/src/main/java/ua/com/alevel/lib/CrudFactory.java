...
public class CrudFactory {
    ...
    public CrudService getCurrent(String implementation) {
        for (Class<? extends CrudService> aClass : crudServiceList) {
            if (!aClass.isAnnotationPresent(Deprecated.class)&& aClass.getSimpleName().equalsIgnoreCase(implementation)) {
                try {
